...
if(isset($_FILES["upfile"])){
    ...
    for($i=0;$i<$count;$i++){
        ...
        if(!empty($files["name"][$i])){
            //변수$upfile_name[$i]에 $files name의[$i] 갯수를 넣는다.
            $upfile_name[$i]=$files["name"][$i];
            $upfile_tmp_name[$i] = $files["file_name"][$i];
...
if(isset($mode) && $mode === "modify"){ //글수정
    ...
    $sql="insert into $table (id,name,subject,content,regist_day,hit,file_name_0,file_name_1,file_name_2,file_copied_0,file_copied_1,file_copied_2)";
    ...
    for($i=0;$i<$count;$i++){
        if($files["name"][$i]!=""){
          //$files["name"][$i]이 있다면
          //그 파일즈네임이 마지막순번째라면 $copied_file_name[$i]값을 주고 )닫아준다.
          //마지막순번째가 아니라면 값을 계속 넣어주다가 마지막 순번째가 됏을때 위로올라가 )닫아준다.
            if($i==$count-1){
...
echo ("
 <script>
    location.href='./cm_free_exhibit.php?table=$table&page=$page';
 </script>
 ");
?>
