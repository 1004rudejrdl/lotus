...
define('SCALE', 10);
$sql=$result=$total_record=$total_page=$start=$maching="";
$row="";
...
if(isset($_GET["mode"])&&$_GET["mode"]=="search"){
  ...
  $sql="SELECT * from `commu` where board_type = 'r' order by group_num desc";
    ...
    <script src="../../js/effect_common.js"></script>
      ...
      <div class="main_body">
        ...
        <div class="list_search_bar">
          <form name="board_form" action="cm_rv_list.php?mode=search" method="post">
        ...
        <tr class="submain_list_item">
          ...
          <td class="li_con_sbj"><a href="./cm_rv_view.php?num=<?=$num?>&page=<?=$page?>&hit=<?=$hit+1?>"><?=$subject?></a></td>
      ...
      <div>
        ...
        <?php
        $sql="SELECT matching from `member_meeting` where id = '$userid'"; //지금 로그인된 값만 확인해서 매칭값만 뽑아옴
          $result = mysqli_query($conn,$sql);
          if (!$result) {
            ...
            // echo "<script>alert('$maching');</script>";
      ...
      <a href="./cm_rv_list.php?page=1">◀◀</a>
      <?php
      if ($page>1) {
          $page_go=$page-1;
           echo '<a class="previous" href="./cm_rv_list.php?page='.$page_go.'">이전 ◀</a>';
         }else {
           echo '<a class="previous" href="./cm_rv_list.php?page=1">이전 ◀</a>';
         }
         for ($i=1; $i <= $total_page ; $i++) {
           if($page==$i){
             ...
             //더블은 문자로 인식
             echo "<a href='./cm_rv_list.php?page=$i'>&nbsp;$i&nbsp;</a>";
           }
         }
         if ($total_page==0) {
           echo '<a class="next" href="./cm_rv_list.php?page=1">▶ 다음</a>';
         }elseif ($page+1>$total_page) {
           $page_end=$total_page;
           echo '<a class="next" href="./cm_rv_list.php?page='.$page_end.'">▶ 다음</a>';
         }else{
           $page_go=$page+1;
           echo '<a class="next" href="./cm_rv_list.php?page='.$page_go.'">▶ 다음</a>';
         }
         ?>
      <a href="./cm_rv_list.php?page=<?=$total_page?>">▶▶</a>
